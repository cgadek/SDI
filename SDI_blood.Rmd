...
df.cont%>%
  filter(hct_cont != "at reference")%>%
  ggplot(aes(log10(gen_egg_mass), SSDI_hb, color=hct_cont))+
  geom_point()+
  geom_smooth(method="lm")


df.blood%>%
  ggplot(aes(mod_adult_survival, mean_hct_female, color=log10(mean_mass_female)))+
  geom_point()+
  geom_smooth(method="lm")
...
df.cont%>%
  ggplot(.,aes(mean_mass_male, SSDI_hb))+
  geom_point()+
  geom_smooth(method="lm")
