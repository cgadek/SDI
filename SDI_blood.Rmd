...
contr.col <- c(h2[1], h2[8], h1[3], h4[10])
...
df.all <- read_csv("data/all_blood_final.csv")

df.within <- read_csv("data/blood_final_within.csv")
...
Keeping this data processing separate, mainly to allow for changing initial sample size.
```{r}
...
df.contribution <- df %>%
  filter(n_hb_male >= thresh,
         n_hb_female >= thresh) %>%
  mutate(SDBI_hb_z = scale(SSDI_hb),
         SDBI_hct_z = scale(SSDI_hct),
         expect_SBDI_hb_z = mean(SDBI_hb_z[which(abs(SDBI_hb_z) <= 0.25)], na.rm = T),# calculate the pooled expectation from the mean of all z scores  within 0.25 sd 
         expect_SBDI_hct_z = mean(SDBI_hct_z[which(abs(SDBI_hct_z) <= 0.25)], na.rm = T),# calculate the pooled expectation from the mean of all z scores  within 0.25 sd 
         expect_SBDI_hb_elev_z = mean(mean_sample_elev[which(abs(SDBI_hb_z) <= 0.25)], na.rm = T),
         expect_SBDI_hct_elev_z = mean(mean_sample_elev[which(abs(SDBI_hct_z) <= 0.25)], na.rm = T),
         species_0_SDI_hb = species[which.min(abs(SSDI_hb) - abs(expect_SBDI_hb_z))][1],
         species_0_SDI_hct = species[which.min(abs(SSDI_hct) - abs(expect_SBDI_hct_z))][1],
    male_0_SDI_hb = mean(mean_hb_male[which.min(abs(SSDI_hb)- abs(expect_SBDI_hb_z))], na.rm = T),
    #I believe these result in list of values so I will take a mean from these
    female_0_SDI_hb = mean(mean_hb_female[which.min(abs(SSDI_hb)- abs(expect_SBDI_hb_z))], na.rm =
                             T),
     male_0_SDI_hct = mean(mean_hct_male[which.min(abs(SSDI_hct)- abs(expect_SBDI_hct_z))], na.rm = T),
    #I believe these result in list of values so I will take a mean from these
    female_0_SDI_hct = mean(mean_hct_female[which.min(abs(SSDI_hct)- abs(expect_SBDI_hct_z))], na.rm =
                             T),
    pooled_diff_from_expected_hb_male = mean_hb_male - male_0_SDI_hb, #these are from subtracting all mean hb values from a single species closest to the central SDI from z scores above
    pooled_diff_from_expected_hb_female = mean_hb_female - female_0_SDI_hb,
     pooled_diff_from_expected_hct_male = mean_hct_male - male_0_SDI_hct, #these are from subtracting all mean hb values from a single species closest to the central SDI from z scores above
    pooled_diff_from_expected_hct_female = mean_hct_female - female_0_SDI_hct,
     pooled_diff_from_expected_elev_hb = mean_sample_elev - expect_SBDI_hb_elev_z,
     pooled_diff_from_expected_elev_hct = mean_sample_elev - expect_SBDI_hct_elev_z,
     pooled_diff_from_exp_hb_male_elev_corr = pooled_diff_from_expected_hb_male + (pooled_diff_from_expected_elev_hb * (0.7/
                                                                          1000)),
    #need to correct for ~ 1 unit increase in per 1000 m increase in elevation
    pooled_diff_from_exp_hb_female_elev_corr = pooled_diff_from_expected_hb_female + (pooled_diff_from_expected_elev_hb * (0.7/
                                                                          1000)),
    
    pooled_diff_from_exp_hct_male_elev_corr = pooled_diff_from_expected_hct_male + (pooled_diff_from_expected_elev_hct * (0.02/
                                                                          1000)),
    #need to correct for ~ 1 unit increase in per 1000 m increase in elevation
    pooled_diff_from_exp_hct_female_elev_corr = pooled_diff_from_expected_hct_female + (pooled_diff_from_expected_elev_hct * (0.02/
                                                                          1000)),
     pooled_hb_SBD_driver = if_else(
      abs(pooled_diff_from_exp_hb_male_elev_corr) < abs(pooled_diff_from_exp_hb_female_elev_corr) &
        pooled_diff_from_exp_hb_female_elev_corr < 0,
      ...
      if_else(
        ...
        if_else(
          abs(pooled_diff_from_exp_hb_male_elev_corr) > abs(pooled_diff_from_exp_hb_female_elev_corr) &
            pooled_diff_from_exp_hb_male_elev_corr < 0,
     ...
     pooled_hct_SBD_driver = if_else(
      abs(pooled_diff_from_exp_hct_male_elev_corr) < abs(pooled_diff_from_exp_hct_female_elev_corr) &
        pooled_diff_from_exp_hct_female_elev_corr < 0,
      "female driven reduction",
      if_else(
        abs(pooled_diff_from_exp_hct_male_elev_corr) < abs(pooled_diff_from_exp_hct_female_elev_corr) &
          pooled_diff_from_exp_hct_female_elev_corr > 0,
        ...
        if_else(
          abs(pooled_diff_from_exp_hct_male_elev_corr) > abs(pooled_diff_from_exp_hct_female_elev_corr) &
            pooled_diff_from_exp_hct_male_elev_corr < 0,
          "male driven reduction",
          if_else(
            abs(pooled_diff_from_exp_hct_male_elev_corr) > abs(pooled_diff_from_exp_hct_female_elev_corr) &
              pooled_diff_from_exp_hct_male_elev_corr > 0,
  ...
  group_by(family) %>%
  mutate(
    fam_species_0_SDI_hb = species[which.min(abs(SSDI_hb) - abs(expect_SBDI_hb_z))][1],
    fam_elev_0_SDI_hb = mean_sample_elev[which.min(abs(SSDI_hb) - abs(expect_SBDI_hb_z))][1],
    fam_male_0_SDI_hb = mean(mean_hb_male[which.min(abs(SSDI_hb)- abs(expect_SBDI_hb_z))], na.rm = T),
    #I believe these result in list of values so I will take a mean from these
    fam_female_0_SDI_hb = mean(mean_hb_female[which.min(abs(SSDI_hb)- abs(expect_SBDI_hb_z))], na.rm =
                             T),
    fam_diff_from_expected_hb_male = mean_hb_male - fam_male_0_SDI_hb,
    fam_diff_from_expected_hb_female = mean_hb_female - fam_female_0_SDI_hb,
   
    fam_diff_from_expected_elev = mean_sample_elev - fam_elev_0_SDI_hb,
    fam_diff_from_exp_hb_male_elev_corr = fam_diff_from_expected_hb_male + (fam_diff_from_expected_elev * (0.7/                                                                   1000)),
    #need to correct for ~ 1 unit increase in per 1000 m increase in elevation
   fam_diff_from_exp_hb_female_elev_corr = fam_diff_from_expected_hb_female + (fam_diff_from_expected_elev * (0.7/                                                                   1000)),
    fam_SBD_hb_driver = if_else(
      abs(fam_diff_from_exp_hb_male_elev_corr) < abs(fam_diff_from_exp_hb_female_elev_corr) &
        fam_diff_from_exp_hb_female_elev_corr < 0,
      "female driven reduction",
      if_else(
        abs(fam_diff_from_exp_hb_male_elev_corr) < abs(fam_diff_from_exp_hb_female_elev_corr) &
          fam_diff_from_exp_hb_female_elev_corr > 0,
        ...
        if_else(
          abs(fam_diff_from_exp_hb_male_elev_corr) > abs(fam_diff_from_exp_hb_female_elev_corr) &
            fam_diff_from_exp_hb_male_elev_corr < 0,
          "male driven reduction",
          if_else(
            abs(fam_diff_from_exp_hb_male_elev_corr) > abs(fam_diff_from_exp_hb_female_elev_corr) &
              fam_diff_from_exp_hb_male_elev_corr > 0,
   ...
   fam_species_0_SDI_hct = species[which.min(abs(SSDI_hct) - abs(expect_SBDI_hct_z))][1],
    fam_elev_0_SDI_hct = mean_sample_elev[which.min(abs(SSDI_hct) - abs(expect_SBDI_hct_z))][1],
    fam_male_0_SDI_hct = mean(mean_hct_male[which.min(abs(SSDI_hct)- abs(expect_SBDI_hct_z))], na.rm = T),
    #I believe these result in list of values so I will take a mean from these
    fam_female_0_SDI_hct = mean(mean_hct_female[which.min(abs(SSDI_hct)- abs(expect_SBDI_hct_z))], na.rm =
                             T),
    fam_diff_from_expected_hct_male = mean_hct_male - fam_male_0_SDI_hct,
    fam_diff_from_expected_hct_female = mean_hct_female - fam_female_0_SDI_hct,
   
    fam_diff_from_expected_elev = mean_sample_elev - fam_elev_0_SDI_hct,
    fam_diff_from_exp_hct_male_elev_corr = fam_diff_from_expected_hct_male + (fam_diff_from_expected_elev * (0.02/                                                                   1000)),
    #need to correct for ~ 1 unit increase in per 1000 m increase in elevation
   fam_diff_from_exp_hct_female_elev_corr = fam_diff_from_expected_hct_female + (fam_diff_from_expected_elev * (0.02/                                                                   1000)),
    fam_SBD_hct_driver = if_else(
      abs(fam_diff_from_exp_hct_male_elev_corr) < abs(fam_diff_from_exp_hct_female_elev_corr) &
        fam_diff_from_exp_hct_female_elev_corr < 0,
      "female driven reduction",
      if_else(
        abs(fam_diff_from_exp_hct_male_elev_corr) < abs(fam_diff_from_exp_hct_female_elev_corr) &
          fam_diff_from_exp_hct_female_elev_corr > 0,
        ...
        if_else(
          abs(fam_diff_from_exp_hct_male_elev_corr) > abs(fam_diff_from_exp_hct_female_elev_corr) &
            fam_diff_from_exp_hct_male_elev_corr < 0,
          "male driven reduction",
          if_else(
            abs(fam_diff_from_exp_hct_male_elev_corr) > abs(fam_diff_from_exp_hct_female_elev_corr) &
              fam_diff_from_exp_hct_male_elev_corr > 0,
...
```{r}
df.contribution%>%
  group_by(pooled_hct_SBD_driver)%>%
  dplyr::summarise(n=n())
```
...
df.cont%>%
  ...
  ggplot(aes(SSDI_hb, diff_from_expected_hb_male_base))+
  geom_smooth(color="black", method="lm")+
  geom_point(color=mycolors[1], size=2, alpha=0.5)+
  geom_hline(yintercept = 0, linetype="dashed")
  labs(x="[Hb] SDI", y= "Difference from\n expected [Hb]")

df.cont%>%
  filter(hb_cont != "at reference")%>%
  ggplot(aes(SSDI_hb, diff_from_expected_hb_male_base, color=dimorphic_01))+
  geom_smooth(method="lm")+
  geom_point()

df.cont%>%
  filter(hb_cont != "at reference")%>%
  ggplot(aes(SSDI_hb, diff_from_expected_hb_male_base, color=log10(mean_mass_male)))+
  geom_smooth(method="lm")+
  geom_point()

 df.cont%>%
  filter(hct_cont != "at reference")%>%
  ggplot(aes(SSDI_hct, diff_from_exp_hct_elev_corr_female, color=hct_cont))+
  geom_smooth(method="lm")+
  geom_point()

df.cont%>%
  filter(hct_cont != "at reference")%>%
  ggplot(aes(SSDI_hct, hct_exp_diff_global_mean, color=SSDI_hb))+
  geom_smooth()+
  geom_point()

df.cont%>%
  filter(hct_cont != "at reference")%>%
  ggplot(aes(SSDI_hct, hct_exp_diff_base, color=SSDI_hb))+
  geom_smooth()+
  geom_point()

df.cont%>%
  filter(hb_cont != "at reference")%>%
  ggplot(aes(mean_sample_elev, hb_exp_diff))+
  geom_smooth(method="lm")+
  geom_point(aes(color=hb_cont))

df.cont%>%
  filter(hct_cont != "at reference")%>%
  ggplot(aes(mean_sample_elev, hct_exp_diff))+
  geom_smooth(method="lm")+
  geom_point()

df.blood%>%
  ggplot(aes(log10(gen_egg_mass), SSDI_hb))+
  geom_smooth(method="lm")+
  geom_point()
...
p<-df.contribution%>%
   filter(SSDI_hb <=0.5,
        !is.na(pooled_hb_SBD_driver))%>%
ggplot(., aes(pooled_diff_from_exp_hb_female_elev_corr, pooled_diff_from_exp_hb_male_elev_corr))+
  ggdensity::geom_hdr(n=100, probs=c( 0.25, 0.50, 0.75, 0.95))+
  geom_point(aes(color=pooled_hb_SBD_driver),alpha=0.5)+
...
p<-df.contribution%>%
  ...
  mutate(log10_fem_mass = log10(mean_mass_female),
         log10_spec_gen_egg_mass = log10(spec_gen_egg_mass),
         log10_repro_mass = log10(spec_gen_clutch_size * spec_gen_egg_mass),
         log10_fem_mass = if_else(is.na(log10_fem_mass) | log10_fem_mass=="-Inf",NA, log10_fem_mass),
         log10_spec_gen_egg_mass = if_else(is.na(log10_spec_gen_egg_mass) | log10_spec_gen_egg_mass=="-Inf",NA, log10_spec_gen_egg_mass),
          log10_repro_mass = if_else(is.na(log10_repro_mass) | log10_repro_mass=="-Inf",NA, log10_repro_mass),
         resids.egg.mass = resid(lm(log10_spec_gen_egg_mass ~ log10_fem_mass, cur_data() %>% dplyr::select(log10_spec_gen_egg_mass, log10_fem_mass), na.action = na.exclude)),
...
ggplot(., aes(resids.repro.mass, SSDI_hct))+
  geom_point(alpha=0.6,  shape=19, color="grey")+
  geom_smooth(method="lm", color="black")+ 
  labs(x= "Residuals (egg mass (g) x clutch size) \n ~ body size (g)")+
  ylab(bquote(SDI[Hct]))+
  geom_hline(yintercept = 0, linetype = "dashed")+
  theme(legend.title = element_blank(),
        legend.position = "none", 
...
p<-df.contribution%>%
  ...
  mutate(log10_fem_mass = log10(mean_mass_female),
         ...
         log10_spec_gen_egg_mass = if_else(is.na(log10_spec_gen_egg_mass) | log10_spec_gen_egg_mass=="-Inf",NA, log10_spec_gen_egg_mass),
         resids.egg.mass = resid(lm(log10_spec_gen_egg_mass ~ log10_fem_mass, cur_data() %>% dplyr::select(log10_spec_gen_egg_mass, log10_fem_mass), na.action = na.exclude)))%>%
   filter(fam_SBD_hb_driver %in% c("female driven reduction", "female driven increase"))%>%
ggplot(., aes(resids.egg.mass, fam_diff_from_exp_hb_female_elev_corr))+
  geom_point(alpha=0.6,  shape=19, color="grey")+
  geom_smooth(method="lm", color="black")+ 
...
p<-df.contribution%>%
  ...
  mutate(log10_fem_mass = log10(mean_mass_female),
         ...
         log10_spec_gen_egg_mass = if_else(is.na(log10_spec_gen_egg_mass) | log10_spec_gen_egg_mass=="-Inf",NA, log10_spec_gen_egg_mass),
         resids.egg.mass = resid(lm(log10_spec_gen_egg_mass ~ log10_fem_mass, cur_data() %>% dplyr::select(log10_spec_gen_egg_mass, log10_fem_mass), na.action = na.exclude)))%>%
   filter(fam_SBD_hb_driver == "male driven increase")%>%
ggplot(., aes(resids.egg.mass, fam_diff_from_exp_hb_male_elev_corr))+
  geom_point(alpha=0.6,  shape=19, color="grey")+
  geom_smooth(method="lm", color="black")+ 
...
p<-df.contribution%>%
  ...
  mutate(log10_fem_mass = log10(mean_mass_female),
         ...
         log10_spec_gen_egg_mass = if_else(is.na(log10_spec_gen_egg_mass) | log10_spec_gen_egg_mass=="-Inf",NA, log10_spec_gen_egg_mass),
         resids.egg.mass = resid(lm(log10_spec_gen_egg_mass ~ log10_fem_mass, cur_data() %>% dplyr::select(log10_spec_gen_egg_mass, log10_fem_mass), na.action = na.exclude)))%>%
   filter(fam_SBD_hct_driver == "female driven reduction")%>%
ggplot(., aes(resids.egg.mass, fam_diff_from_exp_hct_female_elev_corr))+
  geom_point(alpha=0.6,  shape=19, color="grey")+
  geom_smooth(method="lm", color="black")+ 
...
p<-df.contribution%>%
  ...
  mutate(log10_fem_mass = log10(mean_mass_female),
         ...
         log10_spec_gen_egg_mass = if_else(is.na(log10_spec_gen_egg_mass) | log10_spec_gen_egg_mass=="-Inf",NA, log10_spec_gen_egg_mass),
         resids.egg.mass = resid(lm(log10_spec_gen_egg_mass ~ log10_fem_mass, cur_data() %>% dplyr::select(log10_spec_gen_egg_mass, log10_fem_mass), na.action = na.exclude)))%>%
   filter(fam_SBD_hct_driver == "male driven increase")%>%
ggplot(., aes(resids.egg.mass, fam_diff_from_exp_hct_male_elev_corr))+
  geom_point(alpha=0.6,  shape=19, color="grey")+
  geom_smooth(method="lm", color="black")+ 
...
hb.cont <- df.contribution%>%
  filter(SSDI_hb <=0.8)%>%
  dplyr::select(fam_SBD_hb_driver, birdtree)%>%
  na.omit()%>%
  column_to_rownames(var="birdtree")%>%
  pull(fam_SBD_hb_driver)%>%
  as.factor()

hb.cont <- setNames(hb.cont, df.contribution%>%
  dplyr::select(fam_SBD_hb_driver, birdtree)%>%
  na.omit()%>%
  pull(birdtree))
...
map.trees<-make.simmap(t,hb.cont,model="ER",
    nsim=100)
