...
tree <- read.tree("data/birds_mcc.tre") #need to trim to model. will take a million years to run with 9000+ tips anyway
```
...
```{r}
p1 <- Dunning2%>%
  ggplot(., aes(x=log10(min_elev), y=SSDI_con))+
  #geom_point(alpha=0.3)+
  geom_smooth( se=T,color="black", linetype="dashed")+
  theme(legend.position = "none")
...
p2 <- Dunning2%>%
  #filter(family %in% c("Trochilidae", "Furnariidae", "Tyrannidae", "Thraupidae"))%>%
  ggplot(., aes(x=log10(max_elev), y=SSDI_con))+
  #geom_point(alpha=0.3)+
  geom_smooth( se=T, color="black", linetype="dashed")+
  theme(legend.position = "none")
...
p3 <- Dunning2%>%
  #filter(family %in% c("Trochilidae", "Furnariidae", "Tyrannidae", "Thraupidae"))%>%
  ggplot(., aes(x=log10(mdpt), y=SSDI_con))+
  #geom_point(alpha=0.3)+
  geom_smooth( se=T, color="black", linetype="dashed")+
  theme(legend.position = "none")

p4 <-ggplot(Dunning2, aes(x=log10(amp), y=SSDI_con))+
  #geom_point(alpha=0.3)+
  geom_smooth( se=T, color="black", linetype="dashed")+
  theme(legend.position = "none")
